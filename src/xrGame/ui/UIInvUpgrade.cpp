...
void UIUpgrade::set_texture(Layer layer, LPCSTR texture)
{
	switch (layer)
	{
		case LAYER_ITEM:  
		{
			VERIFY(texture); m_item->InitTexture(texture);
			break;
		}
		case LAYER_BORDER:
		{
...
void UIUpgrade::OnClick()
{
	...
	{
		m_parent_wnd->AskUsing(make_string<const char*>("%s %s", g_pStringTable->translate("st_upgrade_install").c_str(), get_upgrade()->name()), get_upgrade()->id_str());
	}
