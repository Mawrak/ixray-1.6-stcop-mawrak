    ...
    case eFire2:
    {
		...
		if (attackStarted)
        {
            ...
            if (state == eFire && HudAnimationExist("anim_shoot1_end"))
                time = PlayHUDMotion("anim_shoot1_end", EHudMixType::eNoMix, state);
            else if (HudAnimationExist("anim_shoot2_end")) // eFire2
                time = PlayHUDMotion("anim_shoot2_end", EHudMixType::eNoMix, state);
...
void CWeaponKnife::switch2_Attacking(u32 state)
{
	if (state == eFire)
	{
		...
		PlayHUDMotion(HudAnimationExist("anm_attack") ? "anm_attack" : "anim_shoot1_start", EHudMixType::eNoMix, state);
	...
	else
	{
		...
		PlayHUDMotion(HudAnimationExist("anm_attack2") ? "anm_attack2" : "anim_shoot2_start", EHudMixType::eNoMix, state);
...
void CWeaponKnife::switch2_Showing	()
{
	...
	PlayHUDMotion(HudAnimationExist("anm_show") ? "anm_show" : "anim_draw", EHudMixType::eNoMix, GetState());
